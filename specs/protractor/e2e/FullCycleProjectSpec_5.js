...
describe('Full cycle project test', function () {
	...
	var mo_rate_for_partner = param.large_mo_rate * 1.05;
	var proj_duration = 12;
	var number_of_pays = 26;
	...
	it('A member and sets pay rate', function () {
		...
		payratesact.set_pay_rate(currency, param.hourly_rate, param.daily_rate, param.weekly_rate, param.large_mo_rate);
		payRatesPage.rates.each(function (item) {
			...
			expect(Number(user_rates[3].substring(0, user_rates[3].indexOf(currency)))).toEqual(param.large_mo_rate);
		});
		loginact.log_out();
	});
	...
	it('Log in as a partner and create a new project with skills', function () {
		...
		projact.post_project_fn('Project_1', 'Project description', project_skill, 2, 'mo', proj_duration, helper.getDate(+3), helper.getDate(+13), false, null, null, null, 0);
	...
	it('As a partner I assign the project and set recurring payment', function () {
		...
		comact.get_table_element(6, 0).getText().then(function (text) {
			...
			expect(Number(newText.replace(/,/g, ''))).toEqual(proj_duration * mo_rate_for_partner);
		});
		//check available actions
		comact.expand_fn(0);
	...
	it('As a member I check, that project is visible in my projects and has status assigned', function () {
		...
		comact.get_table_row(2, 0).then(function (text) {
			...
			expect(proj_sum.replace(',', '')).toEqual(Number(proj_duration * param.large_mo_rate).toFixed(2) + ' ' + currency);
		});
		loginact.log_out();
	});
	it('As a member I request first payment for the project', function () {
		...
		loginact.log_in(cred.member_vat_uk_email, cred.pswd);
		dashact.view_payments();
		payact.user_requests_payment(full_project_id, proj_duration * param.large_mo_rate / number_of_pays * 1.2, currency, true);
	...
	it('As a partner I approve payment for the project', function () {
		...
		loginact.log_in(cred.part_vat_uk_email, cred.pswd);
		dashact.view_payments();
		payact.partner_approves_requested_pay(full_project_id, currency, mo_rate_for_partner * proj_duration / number_of_pays);
	...
	it('Log in as a member and check invoice', function () {
		...
		comact.get_table_element(4, 0).then(function (text) {
			...
			var pr_sum = (param.large_mo_rate * proj_duration / number_of_pays).toFixed(2) + ' ' + currency;
	...
	it('Log in as a partner and check invoice sum', function () {
		...
		comact.get_table_element(4, 0).then(function (text) {
			console.log('sum info: ' + text);
			var pr_sum = (mo_rate_for_partner * proj_duration / number_of_pays).toFixed(2) + ' ' + currency;
			expect(pr_sum).toEqual(text);
		});
	...
	it('Log in in xero and check partner invoice', function () {
		...
		xero.get_table_cell(8, 1).then(function (text) {
			...
			expect(Number(text.replace(',', '')).toFixed(2)).toEqual((mo_rate_for_partner * proj_duration / number_of_pays).toFixed(2));
		});
	});
	it('Check member invoice in xero', function () {
		...
		xero.get_table_cell(7, 1).then(function (text) {
			...
			expect(Number(text.replace(',', '')).toFixed(2)).toEqual(Number(param.large_mo_rate * proj_duration / Number(number_of_pays)).toFixed(2));
		});
		xero.log_out();
	});
});