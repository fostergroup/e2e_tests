...
describe('Full cycle project test', function () {
	...
	it('A partner sets profile payment to invalid credit card', function () {
	...
	it('As a partner I assign the project and set one off payment', function () {
		...
		expect(findMembers.assign_btn.isDisplayed()).toBeTruthy();
		expect(findMembers.invite_btn.isDisplayed()).toBeTruthy();
		projact.partner_assigns_project(1, hourly_rate_for_partner, daily_rate_for_partner, weekly_rate_for_partner, mo_rate_for_partner, currency, true, false);
		findact.back_from_modal();
		browser.sleep(1000);
		...
		comact.get_table_element(3, 0).getText().then(function (text) {
			console.log('expected project status: ' + text);
			expect(text).toEqual(status.pend_paym);
		});
		...
		//check project assignee:
		comact.get_table_element(7, 0).getText().then(function (text) {
			var newText = text.substring(text.indexOf(currency) + 3, text.length);
			console.log('expected project assignee: ' + newText);
			expect(newText).toContain(cred.with_proj_member_vat_non_uk_id);
			expect(newText).toContain('Testing Account 226 This-Issuchsirname');
		});
		//check available actions
		comact.expand_fn(0);
		browser.sleep(1000);
