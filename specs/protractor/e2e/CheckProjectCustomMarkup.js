...
var MessageActions = require('../../src/actions/MessageActions.js');
...
describe('Full cycle project test', function () {
	...
	var currency = 'USD';
	var proj_markup = 10;
	var hourly_rate_for_partner = param.hourly_rate * (1 + proj_markup / 100);
	var daily_rate_for_partner = param.daily_rate * (1 + proj_markup / 100);
	var weekly_rate_for_partner = param.weekly_rate * (1 + proj_markup / 100);
	var mo_rate_for_partner = param.mo_rate * (1 + proj_markup / 100);
	var proj_duration = 3;
	...
	it('As admin set up custom project markup to 0', function () {
		...
		adminact.set_proj_custom_markup(null, proj_markup, null, pr_id);
		adminact.log_out();
	});
	...
	it('As a partner I find available members to invite him to an interview', function () {
		...
		comact.get_table_element(6, 0).getText().then(function (text) {
			...
			expect(Number(text.replace(/,/g, '')).toFixed(0)).toEqual(Number(weekly_rate_for_partner).toFixed(0));
		});
	...
	it('As invited member I check, that project visible for interview and message a partner', function () {
		...
		//accept interview:
		comact.expand_fn(0);
		intact.send_mes('Comment');
		//mesact.close_msg();
		browser.sleep(2000);
		//accept interview:
		comact.expand_fn(0);
		intact.resched_int(null);
		browser.sleep(3000);
		intact.search_filt(full_project_id);
		intact.view_interview(full_project_id, status.not_avail);
		loginact.log_out();
	});
	...
	it('As a partner I assign the project and set one off payment', function () {
		...
		comact.get_table_element(6, 0).getText().then(function (text) {
			...
			expect(Number(newText.replace(/,/g, '')).toFixed(2)).toEqual(Number(3 * hourly_rate_for_partner).toFixed(2));
		});
		//check available actions
		comact.expand_fn(0);
		browser.sleep(1000);
